...
판단(어느 그림을 몇 %로, 무엇과 나란히)은 네가 하고, 치환은 도구로 한다. 손으로 HTML을 붙여 넣으면 닫는 태그 누락·캡션 중복·엉뚱한 그림 교체가 생기기 쉽다. 두 도구 모두 대상이 정확히 한 번 나오는지 확인한 뒤에만 쓰고, 어긋나면 아무것도 바꾸지 않고 멈춘다. **순서가 중요하다 — 토글 먼저, 레이아웃 나중.** 토글 안의 이미지는 들여써져 있어서 풀어 주기 전에는 레이아웃 도구가 찾지 못한다.

```bash
# 1) PDF에서 ▼로 확인한 토글들을 <details>로 (제목의 일부만 적으면 된다)
python3 .claude/skills/paper-review-convert/scripts/convert_toggle.py <슬러그> "토글 제목" "다른 토글 제목"
# 2) 이미지 레이아웃: spec을 scratchpad에 JSON으로 쓰고 적용 (형식은 apply_layout.py 맨 위 설명 참고)
python3 .claude/skills/paper-review-convert/scripts/apply_layout.py <슬러그> <spec.json>
```

spec은 그림 번호 기준이다: `{"fig": 1, "w": 48, "alt": "..."}`(단독), `{"row": [...]}`(나란히), `"caption": true`(노션이 alt에 넣어 둔 문구를 캡션으로, 본문에 떠 있는 중복 줄은 자동 제거), `{"col": [...], "w": 44}`(한 칸에 그림을 세로로 쌓기). PDF에서 본문 폭의 85% 이상을 차지하는 그림은 전체 폭으로 보고 alt만 바꾼다. alt는 모든 그림에 대해 내용을 설명하는 문장으로 채운다 (스크립트가 넣은 "X figure 3" 같은 자리표시를 남기지 않는다).

도구가 다루지 못하는 배치(그림 옆에 글이나 코드블록, 수식을 두는 2단 구성 등)는 아래 패턴을 보고 직접 쓴다. 도구가 만들어 내는 HTML도 같은 패턴이다. 전체 폭 + 캡션 없는 이미지는 마크다운 `![]()` 그대로 둔다.
...
## 사용자가 정한 것 (2026-09-20)

- **형광펜 강조는 복원하지 않는다.** 노션 md export에는 형광펜 정보가 전혀 남지 않는다. PDF에는 보이지만, 이미지에서 강조 구간의 경계를 눈대중으로 잡아 `<mark>`를 넣는 방식은 정확도가 떨어진다고 사용자가 판단했다. 틀린 곳에 강조가 들어가는 것보다 없는 편이 낫다.
- **토글은 접힌 상태로 둔다.** `<details>`에 `open`을 붙이지 않는다.
- **노션 커스텀 이모지의 대체 아이콘**: 아바타 이모지가 달린 "느낀점"(및 제목 없는 소감) 콜아웃은 😲. 같은 아바타라도 Motivation·핵심요약처럼 다른 굵은 제목이 붙은 콜아웃과, "학습흐름"에 쓰인 다른 커스텀 이모지는 기본값 💡 (학습흐름용 아이콘은 아직 미정 — 해당 논문이 나오면 사용자에게 묻는다). 스크립트가 자동 처리하고 결과를 출력한다. 사용자가 노션에서 직접 고른 일반 이모지(📋 등)는 "느낀점"이어도 그대로 둔다.
...
- **문단 첫 줄의 `|`는 `\|`로 이스케이프한다**: kramdown은 `|`가 든 줄이 문단의 첫 줄이면 표로 해석한다. 링크 제목의 "A | B"를 별도 문단으로 떼어 놓았더니 링크가 두 칸짜리 표로 쪼개진 적이 있다 (Diffusion 헤더). 수식 밖에 쓴 `p(x|z)`로 시작하는 문단도 같은 위험이 있다. `audit_build.py`가 원본과 결과물의 표 개수를 비교해 잡아낸다.
